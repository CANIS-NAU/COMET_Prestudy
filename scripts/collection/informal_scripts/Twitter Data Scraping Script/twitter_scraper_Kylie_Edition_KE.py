...
media = results[0]['includes']['media'] if 'media' in results[0]['includes'] else None
users = results[0]['includes']['users'] if 'users' in results[0]['includes'] else None
...
for tweet in tweets:
        ...
        for media_key in media_keys:
            ...
            for item in media:
                if item['media_key'] == media_key:
                    if item['type'] == 'video':
                        data = {'media_key': media_key, 'type': 'video', 'url': item['variants'][0]['url']}
    ...
    referenced_tweets = tweet['referenced_tweets'] if 'referenced_tweets' in tweet else None

    data = [id, text, attachments, author_id, author_name, author_username, conversation_id, created_at, in_reply_to_user_id, lang, public_metrics, referenced_tweets]
